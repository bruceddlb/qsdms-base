    ...
    public class UserService : IUserService
    {
     ...
     AND u.UserId <> 'System' AND u.EnabledMark = 1 AND u.DeleteMark=0");
        ...
        public string SaveForm(string keyValue, UserEntity userEntity)
        {
            ...
            try
            {
                Base_User usermodel = new Base_User();
                using (var tran = QSDMS_SQLDB.GetInstance().GetTransaction())
                {
                    #region 基本信息
                    if (!string.IsNullOrEmpty(keyValue))
                    {
                        ...
                        usermodel = Base_User.SingleOrDefault("where UserId=@0", keyValue);
                        usermodel = EntityConvertTools.CopyToModel<UserEntity, Base_User>(userEntity, usermodel);
                        usermodel.UserId = keyValue;
                        usermodel.Update();
                    }
                    else
                    {
                        ...
                        usermodel = EntityConvertTools.CopyToModel<UserEntity, Base_User>(userEntity, null);
                        usermodel.Insert();
                    ...
                    Base_UserRelation.Delete("where UserId=@0 and IsDefault=1", usermodel.UserId);
                    ...
                    if (!string.IsNullOrEmpty(usermodel.RoleId))
                    {
                        ...
                        Base_UserRole.Delete("where UserId=@0", usermodel.UserId);
                        string[] roles = usermodel.RoleId.Split(',');
                    ...
                    //岗位
                    if (!string.IsNullOrEmpty(usermodel.DutyId))
                    {
                        userRelationEntitys.Add(new UserRelationEntity
                        {
                    ...
                    //职位
                    if (!string.IsNullOrEmpty(usermodel.PostId))
                    {
                        userRelationEntitys.Add(new UserRelationEntity
                        {
                    ...
                    Base_UserAuthorize.Delete("where UserId=@0", usermodel.UserId);
                    //插入用户对应数据权限
                    if (!string.IsNullOrEmpty(usermodel.AuthorizeDataId))
                    {
                        string[] uthorizeDatas = usermodel.AuthorizeDataId.Split(',');
                        for (int i = 0; i < uthorizeDatas.Length; i++)
                        {
                            ...
                            userAuthorize.UserId = usermodel.UserId;
